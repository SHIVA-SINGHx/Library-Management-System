...
const AdminBooksPage = () => {
  ...
  const canSearchRoll = issueForm.rollNumber.trim().length > 0 && !isStudentSelected;
    ...
    searchTimeoutRef.current = window.setTimeout(async () => {
      try {
        ...
        if (response.ok && data.success) {
          ...
          setMatchingStudents([]);
          setSearchError(data.message || "Unable to search students by roll number.");
        }
      } catch (error) {
  ...
  const handleIssueSubmit = async (event) => {
    ...
    if (!issueForm.userEmail) {
      setFormMessage("Search and select a student by roll number before issuing books.");
      return;
    }
    ...
    setFormMessage(`${result.count} manual book record(s) issued successfully!`);
          ...
          <div className={s.headerFlex}>
            <div>
              ...
              <p className={s.subtitle}>
                Select a student, and manual book entries with book code and the
                active overdue fine rule will be used automatically after the due date.
              </p>
            </div>
            <div className={s.fineRuleBadge}>
              Fine rule: Rs. {fineSettings?.amount ?? 0} per {fineSettings?.interval ?? "day"}
            </div>
          </div>
          ...
          <form className={s.form} onSubmit={handleIssueSubmit}>
            ...
            <div className={s.matchingContainer}>
              ...
              <div className={s.studentList}>
                {isSearching ? (
                  <span className={s.searchingMessage}>Searching for students...</span>
                  ...
                  matchingStudents.map((student) => (
                    ...
                    >
                      ...
                      <span className={s.studentRollSpan}>- {student.rollNumber}</span>
                    </button>
                  ))
                ) : (
            ...
            <div className={s.booksSection}>
              <div className={s.booksHeader}>
                ...
                <button type="button" onClick={addBookDraft} className={s.addBookButton}>
                  <FilePlus2 size={16} />
                  Add Book
                </button>
              </div>
