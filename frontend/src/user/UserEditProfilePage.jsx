...
const UserEditProfilePage = () => {
  ...
  const handleSubmit = async(event) =>{
    ...
    const phoneDigits = (form.phoneNo || "").trim().replace(/\D/g, "");
    ...
    try {
      ...
      if(result.ok){
        ...
        setError(result.error || "Profile updated failed. Please try again later")
      }
  
    } catch (err) {
        setLoading(false);
        setError("An unexpected connection error occurred.", err)
    }
  }
