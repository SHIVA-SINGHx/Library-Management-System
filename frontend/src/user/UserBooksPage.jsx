  ...
  const filteredIssuedBooks = useMemo(() => {
    return currentUserHistory.filter((record) => {
      ...
      const matchesSearch =
        ...
        (record.bookCode && record.bookCode.toLowerCase().includes(term)) ||
        (currentUser?.name && currentUser.name.toLowerCase().includes(term));
        ...
        <div className={s.heroFlex}>
          <div>
            ...
            <h1 className={s.heroTitle}>
              Books cards with richer content and cleaner grouped details
            </h1>
            <p className={s.heroText}>
              Each card now uses a clearer top summary, status badge, content
              chips, and a better medium-card layout so the details feel more
              structured and elegent.
            </p>
          </div>
        </div>
      </section>
